...
class bin_parser {																						//calculates and stores relative bin start and end locations
	...
	char delim;
	void get_delim(ifstream &ifs,string name) {
		char c;
		while(1) {
			ifs.get(c);
			if(ifs.fail()) {
				cout << "Error: no line terminator character found in \"" << name << "\"\n";
				exit(1);
			}
			if(c=='\n') {
				delim='\n';
...
protected:
	...
	string strand;
	char delim;
	char get_delim(ifstream &ifs,string name) {
		char c,d;
		while(1) {
			ifs.get(c);
			if(ifs.fail()) {
...
public:
	...
	file_reader(opt_parser &op) {
		...
		if(op.hits!=NULL) {
			...
			delim=get_delim(one,one_hitlist[0]);
		}
	...
	virtual void read(string &line,string &str,int &ret,int *index) {								//for single hit file passed without -p or -m
		...
		if(!one.good() && (*index)<((int)one_hitlist.size()-1)) {
			...
			delim=get_delim(one,one_hitlist[*index]);
			do {
				getline(one,line,delim);
			} while(line=="" && one.good());
		}
...
public:
	file_reader_two(opt_parser &op) : file_reader(op) {
		...
		strand="plus";
		delim2=get_delim(two,two_hitlist[0]);
		current=&one;
		current_delim=&delim;
	}
	void read(string &line,string &str,int &ret,int *index) {
		...
		if(!current->good()) {
			if(current==&one) {
				...
				delim2=get_delim(two,two_hitlist[*index]);
				strand="plus";
				current=&one;
				current_delim=&delim;
				do {
...
public:
	genelist_parser(opt_parser &op,bin_parser &bp) {
		...
		get_delim(genelist,op.genelist);
		char *pch;
